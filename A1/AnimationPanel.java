 ...
 *  It can get the sorted info based on the getSortInfo() with the ascending order of the shapes' area.
 *  Name: Xiaolin Li
 *	UPI: xli556
 *	==========================================================================================
 */
...
public class AnimationPanel extends JComponent implements Runnable {
	...
	private int currentArcWidth=1, currentArcHeight=1; // the current actWidth and actHeight of roundRectangle
	...
	protected void createNewShape(int x, int y) {
		...
		switch (currentShapeType) { //complete this
			...
			case 3: {
				shapes.add(new MovingCircle(x, y, Math.min(currentWidth, currentHeight), marginWidth, marginHeight, currentBorderColor, currentFillColor, currentPath));
				break;
			}
			case 4: {
				shapes.add(new MovingQuadCircles(x, y, Math.min(currentWidth, currentHeight), marginWidth, marginHeight, currentBorderColor, currentFillColor, currentPath));
				break;
			}
			case 5: {
				shapes.add(new MovingPixelArt(x, y, Math.min(currentWidth, currentHeight), marginWidth, marginHeight, currentBorderColor, currentFillColor, currentPath));
				break;
			}
			case 6: {
				shapes.add(new MovingBounsShape(x, y, currentWidth, currentHeight, currentArcWidth, currentArcHeight, marginWidth, marginHeight, currentBorderColor, currentFillColor, currentPath));
				break;
			}
		}
	}
	...
	// get the current arcWidth
	public int getCurrentArcWidth() {
		return currentArcWidth;
	}

	// get the current arcHeight
	public int getCurrentArcHeight() {
		return currentArcHeight;
	}

	//set the current arcWidth for currently selected bounsShape
	public void setCurrentArcWidth(int aw) {
		currentArcWidth = aw;
		for (MovingShape currentShape: shapes)
			if ( currentShape.isSelected())
				currentShape.setArcWidth(currentArcWidth);
	}

	//set the current arcHeight for currently selected bounsShape
	public void setCurrentArcHeight(int ah) {
		currentArcHeight = ah;
		for (MovingShape currentShape: shapes)
			if ( currentShape.isSelected())
				currentShape.setArcHeight(currentArcHeight);
	}
